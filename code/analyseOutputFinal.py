...
root_node='02'
image_size = [224,224]
# resultat_final_path='D:/PROJET/ISBI_ARTICLE/resultat_final/strategie8/' + dataset_name + '/'
...
#name_result_file=dataset_name+'_result_final_'+base+'_cnn_sigmoid_fused_cycle_tree_skinan.csv'
...
# disp = plot_confusion_matrix(y_test, y_test_predict,
...
for i in range(0,len(y_test)-1):
    if y_test[i]==0:
        y_test_cat=np.append(y_test_cat,'SEK')
    elif y_test[i]==1:
        y_test_cat=np.append(y_test_cat,'MEL')
    else :
        y_test_cat=np.append(y_test_cat,'NEV')
        

y_test_predict_cat=[]
for i in range(0,len(y_test_predict)-1):
    if y_test_predict[i]==0:
        y_test_predict_cat=np.append(y_test_predict_cat,'SEK')
    elif y_test_predict[i]==1:
        y_test_predict_cat=np.append(y_test_predict_cat,'MEL')
    else :
        y_test_predict_cat=np.append(y_test_predict_cat,'NEV')
import pandas as pd
import seaborn as sn
import matplotlib.pyplot as plt
data = {'true_label':   y_test_cat,
        'predict_label': y_test_predict_cat
        }
df = pd.DataFrame(data, columns=['true_label','predict_label'])
confusion_matrix = pd.crosstab(df['true_label'], df['predict_label'], rownames=['True label'], colnames=['Predicted label'])#, margins = True
...
for i in range(len(y_test_score)):
    ind=np.argmax(y_test_score[i])
    y_test_predict_score_max[i]=y_test_score[i,ind]
...
result.to_csv(resultat_final_path+name_result_file)
...
prediction_table= predict_table(y_test_score,y_test)
result_images=get_images_with_sorted_probabilities(prediction_table,
                                         True,
                                         0,
                                         number_of_item,
                                         only_false_predictions=False)
visualize_image(result_images,test_path,test_filenames,message,resultat_final_path,result_img_name)
